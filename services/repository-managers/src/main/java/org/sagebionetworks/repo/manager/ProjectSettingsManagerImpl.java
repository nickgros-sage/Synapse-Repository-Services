...
public class ProjectSettingsManagerImpl implements ProjectSettingsManager {
	...
	private void validateS3BucketOwnership(ExternalS3StorageLocationSetting externalS3StorageLocationSetting, List<String> ownerAliases)
		...
		try {
			...
			if (AmazonErrorCodes.S3_BUCKET_NOT_FOUND.equals(e.getErrorCode())) {
				throw new IllegalArgumentException("Did not find S3 bucket " + bucket + ". " + getExplanation(ownerAliases, bucket, key));
				...
				if (key.equals(OWNER_MARKER)) {
					...
					throw new IllegalArgumentException("Did not find S3 object at key " + key + " from bucket " + bucket + ". If the S3 object is in a folder, please make sure you specify a trailing '/' in the base key. " + getExplanation(ownerAliases, bucket, key));
				}
			} else {
	...
	private void validateGoogleCloudBucketOwnership(ExternalGoogleCloudStorageLocationSetting externalGoogleCloudStorageLocationSetting, List<String> ownerAliases)
			throws IOException, NotFoundException {
		...
		try {
			if (!googleCloudStorageClient.bucketExists(bucket)) {
				throw new IllegalArgumentException("Did not find Google Cloud bucket " + bucket + ". " + getExplanation(ownerAliases, bucket, key));
			}
		...
		if (googleCloudStorageClient.getObject(bucket, key) == null) {
			throw new IllegalArgumentException("Did not find Google Cloud object at key " + key + " from bucket " + bucket + ". If the object is in a folder, please make sure you specify a trailing '/' in the base key. " + getExplanation(ownerAliases, bucket, key));
		}
	...
	void inspectUsername(BufferedReader reader, List<String> expectedAliases, String bucket, String key) throws IOException {
