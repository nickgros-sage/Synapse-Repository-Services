...
public interface SynapseGoogleCloudStorageClient {
	 ...
	 */
	Blob putObject(String bucket, String key, File file) throws IOException;
	 ...
	 * @param key the path/filename of the object to upload
	 * @param file a file containing the contents to upload
	 * @param metadata to attach to the object. Note: this is S3 metadata so not all fields
	 *                 may be configured/compatible. This exists for compatibility reasons
	 * @return the uploaded object
	 * @throws IOException if the file cannot be opened
	 */
	@Deprecated
	Blob putObject(String bucket, String key, File file, ObjectMetadata metadata) throws IOException;
